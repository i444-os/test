...
from burp import (IBurpExtender, IHttpListener, ITab, IExtensionStateListener, 
                  IBurpExtenderCallbacks, ISessionHandlingAction)
...
class BurpExtender(IBurpExtender, IHttpListener, ITab, IExtensionStateListener, ISessionHandlingAction):
    ...
    def performAction(self, currentRequest, macroItems):
        ...
        try:
            ...
            if strip_list or add_auth_headers or replace_cookies:
                new_request = self._modify_request(req_bytes, strip_list, add_auth_headers, replace_cookies)
    ...
    def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
        if not self._running or messageIsRequest:
            return
    ...
    def _modify_request(self, request_bytes, strip_headers_lower, add_headers, replace_cookies):
        try:
                    ...
                    if h.lower().startswith("cookie:"):
                        ...
                        for part in cookie_val.split(";"):
                            if "=" in part:
                                k, v = part.split("=", 1)
                                k_strip = k.strip()
                                if k_strip.upper() in replace_cookies:
                                    new_parts.append("%s=%s" % (k_strip, replace_cookies[k_strip.upper()]))
                                    found = True
                                else:
                                    new_parts.append("%s=%s" % (k_strip, v.strip()))
                            else:
                                if part.strip(): new_parts.append(part.strip())
                                
                        if found:
                            headers[i] = "Cookie: %s" % "; ".join(new_parts)
                        break
